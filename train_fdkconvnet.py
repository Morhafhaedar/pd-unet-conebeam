...
class FDKConvnet(LightningModule):
    ...
    def forward(self, *args, **kwargs):
        under_reco = args[0]/LungDataset.percentile
        return (under_reco + self.net(under_reco))*LungDataset.percentile

    def training_step(self, *args, **kwargs):
        batch = args[0]
        sparse_sino, fulldata = batch
        prediction = self(fdk_reconstruction(sparse_sino, self.radon, 'hann'))
...
def main():
    ...
    subtype: str = 'sparse'
    sparsity: int = 4
    run_name: str = f'{subtype}{sparsity}_fdkconv'
    num_epochs: int = 151
    use_amp: bool = True
